...
1. `pnpm check`：通过；14 个测试文件、225 个测试通过，包含 SQLite/打包专项 4 个文件、28 个测试；lint、format、全 workspace typecheck 与 build 通过。迁移恢复测试证明备份发生在任何 Schema DDL 前且备份失败不改变原库；并发测试覆盖双连接 revision CAS，快照测试覆盖删除后的连接状态原始结果重放和严格字段白名单。
