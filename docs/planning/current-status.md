...
Task 11 仓库门禁：`pnpm check` 通过，72 个测试文件、685 个测试通过，类型检查和生产构建通过。

Task 12 已完成：

- Main/Preload 新增下课、选择课后动作和完成复习三条显式 IPC；下课总结复用课程 operation registry，因此长时间 AI 总结可取消，取消结果会持久化并可重新尝试。
- 对话底部新增下课确认、AI 记忆生成状态、取消、失败重试、课程记忆、立即复习/先休息和复习回答保存界面；总结与复习期间不再显示普通课堂输入框。
...
- 设置中心继续提供剩余上下文阈值（10–50%，默认 30%）与最近原文保留轮数（1–50）配置。
- 课堂回复期间仅在输入区附近显示轻量“正在检查上下文并生成导师回复”状态，不把压缩作业或技术错误插入正常聊天消息流。
- 课堂信息的技术页展示 active 快照模型、版本、压缩前剩余比例、阈值以及最近作业的成功、失败或取消状态。
- 普通发送和失败重试共用压缩检查；现有生成取消按钮中止共享 token，因此可取消正在进行的压缩或导师回复。
- SQLite 课程读取只在存在快照或压缩作业时附带诊断数据，历史课程与旧测试保持兼容。

Task 16 / Stage 5 仓库门禁：`pnpm check` 通过，80 个测试文件、719 个测试通过，类型检查和生产构建通过。下一步是最终 E2E、打包与发布验收。
