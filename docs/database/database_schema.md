...
Migration 13 (`lesson_mastery_evidence`) 为 D6-MVP 增加课堂学习诊断持久化。当前实现不写入完整评分 rubric；它只保存成功课堂回答产生的 deterministic 掌握证据和可选误区信号。Provider/generator 失败或取消时不生成诊断记录。
...
### 5.0.14 Review Scheduler MVP（Migration 14）

Migration 14 (`lesson_review_scheduler`) 把 `suggested_review = 1` 的课堂诊断转化为可执行的复习任务。当前切片只做 lesson-scoped 最小闭环：自动创建 `lesson_review_items`，记录 `lesson_review_events`，并按 `remembered` / `forgot` 更新下次到期时间；不实现独立复习中心、通知或复杂 SM-2 调度。
...
| lesson_id               | TEXT | FK `lesson_sessions(id)` ON DELETE CASCADE               | 所属课堂会话                          |
| mastery_evidence_id     | TEXT | FK `lesson_mastery_evidence(id)` ON DELETE CASCADE       | 来源诊断证据                          |
| misconception_signal_id | TEXT | FK `lesson_misconception_signals(id)` ON DELETE SET NULL | 可选关联误区信号                      |
| prompt                  | TEXT | NOT NULL                                                 | 复习提示                              |
| answer_outline_json     | TEXT | NOT NULL                                                 | JSON 字符串数组，保存 1..5 条复习要点 |
| status                  | TEXT | NOT NULL                                                 | `active/completed/suspended`          |
| due_at                  | TEXT | NOT NULL                                                 | 下次复习时间                          |
| created_at              | TEXT | NOT NULL                                                 | 创建时间                              |
| updated_at              | TEXT | NOT NULL                                                 | 最近一次调度更新时间                  |

索引与约束：

- `UNIQUE(mastery_evidence_id)`：同一条掌握证据只生成一个 review item。
- `lesson_review_items_lesson_due`：`(lesson_id, status, due_at)`，用于课堂详情按到期时间读取。
- `lesson_review_items_due`：`(status, due_at)`，为未来全局复习中心预留扫描路径。
- `status` 通过 `CHECK` 约束限制为 `active/completed/suspended`。

#### `lesson_review_events`

| 字段            | 类型 | 约束                                           | 说明                                 |
| --------------- | ---- | ---------------------------------------------- | ------------------------------------ |
| id              | TEXT | PK                                             | Review Event ID                      |
| review_item_id  | TEXT | FK `lesson_review_items(id)` ON DELETE CASCADE | 所属复习任务                         |
| lesson_id       | TEXT | FK `lesson_sessions(id)` ON DELETE CASCADE     | 所属课堂会话，冗余保存便于按课堂读取 |
| rating          | TEXT | NOT NULL                                       | `remembered/forgot`                  |
| response        | TEXT | NOT NULL                                       | 学习者这次复习回答                   |
| previous_due_at | TEXT | NOT NULL                                       | 调度前到期时间                       |
| next_due_at     | TEXT | NULL                                           | 调度后的下一次到期时间               |
| reviewed_at     | TEXT | NOT NULL                                       | 实际复习时间                         |
| created_at      | TEXT | NOT NULL                                       | 创建时间                             |

索引与约束：

- `lesson_review_events_item_reviewed`：`(review_item_id, reviewed_at)`，用于查看单条复习任务历史。
- `lesson_review_events_lesson_reviewed`：`(lesson_id, reviewed_at)`，用于课堂内按时间查看复习记录。
- `rating` 通过 `CHECK` 约束限制为 `remembered/forgot`。
