...
Provider 创建时内部 `revision = 1`。更新使用单条 `UPDATE ... WHERE id = ? AND revision = ?` 执行真实 CAS，并依据受影响行数区分成功及后续的 `stale/not_found`；`updated_at` 仅用于展示和审计。启用在 SQLite immediate 写事务内读取并比较 revision、重新确认目标仍为 Mock 或具有 `secret_ref`，再原子清除旧启用项和切换目标，避免并发事务读到相同 revision 后部分提交。公共 `ProviderProfile` 不暴露 revision。连接测试不写入不可变 request outcome；Task 6 建立 `provider_test_operations`，在 SQLite immediate 写事务中使用独立 `operation_id` 对 `last_test_status` 执行 `testing -> terminal` 的持久化比较并交换转换，使竞争终态确定性串行化，并保存每次成功转换产生的严格校验 Provider 快照；重放先读取操作历史并返回该原始快照，即使当前 Provider 已编辑或删除。Task 8 在此基础上实现外部请求编排与取消。
