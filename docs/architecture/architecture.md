...
引用的页码由 Application 根据已授权 context chunk 补齐，不能信任模型自报位置。Renderer 将引用显示为独立证据卡；“回到来源”只使用课堂已保存的当前文档 `pdf_block` anchor，通过现有受控文档焦点流程延迟加载阅读器，不解析或打开模型提供的任意路径/URL。

PDF 图片抽取位于 Infrastructure：只处理已通过文本层门禁的 PDF，按页匹配受支持图注，优先提取嵌入图片，必要时生成页面渲染回退。Application 在外部抽取前依赖已持久化的导入/文件状态，写入受控资产后以单次仓储事务提交图片元数据和 `ready` 状态；重复调用先读取完成状态，取消或失败时清理本轮已写资产。Renderer 永远不接触本地资产路径。

图片展示沿用归属白名单：Provider 只接收当前文档图片的 `figureId`、页码、图号和图题；Application 校验结构化回复中的每个 `figureId` 均在白名单内。Renderer 通过显式 `documents:get-figure-asset` 请求 `{documentId, figureId}`，Application 再查询当前文档元数据后才读取受控资产，Main 仅返回 `image/png` data URL。未知、跨文档或丢失的资产使用稳定错误，不暴露 `storedPath`。图片卡的“回到图片来源”只传递已验证的文档 ID 与页码给阅读器。
